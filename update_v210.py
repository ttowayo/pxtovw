...
files = [
    ...
    'divider-generator.html', 'aspect-ratio.html', 'css-generator.html', 'gradient-generator.html', 
...
def get_gnb_html(current_file):
    menu_items = [
        ...
        ('aspect-ratio.html', 'Aspect Ratio'),
...
new_note_html = """                  <div class="version-item">
                    ...
                    <ul>
                      <li>Aspect Ratio 비율 계산기 신규 추가</li>
                      <li>양방향 자동 계산 및 Padding Hack 코드 제공</li>
                    </ul>
                  </div>"""
...
for filename in files:
    ...
    # 2. 버전 번호 업데이트 (기존 v2.0.0을 v2.1.0으로)
    content = content.replace('v2.0.0', version_new)
    
    # 3. 업데이트 노트 추가
    note_marker = 'id="close-version-btn"\n                    >\n                      &times;\n                    </button>\n                  </h3>'
    if note_marker in content and 'v2.1.0' not in content:
        content = content.replace(note_marker, note_marker + "\n" + new_note_html)
    
    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)

print('All files updated successfully to v2.1.0')
