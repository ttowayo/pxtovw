...
modal_content = '''                <button id="version-btn" class="version-btn">v2.1.0 <span>▼</span></button>
...
for file in html_files:
    ...
    start_idx = content.find('<div class="version-info">')
    if start_idx == -1:
        continue
    
    # <div class="version-info"> 이후의 첫 </div>\n            </div> 묶음을 찾아야 함
    # version-info를 닫는 </div> 찾기
    i = start_idx
    div_count = 0
    end_idx = -1
    while i < len(content):
    ...
    new_content = content[:start_idx + len('<div class="version-info">\n')] + modal_content + '\n              ' + content[end_idx:]
    
    with open(file, 'w', encoding='utf-8') as f:
        f.write(new_content)
    updated_count += 1

print(f"Fixed version-info in {updated_count} files.")
